...
class Study(Base):
    ...
    def to_json(self):
        return dict((
            convert_to_json("00080020", self.date.strftime("%Y%m%d") if self.date != None else None),
            convert_to_json("00080030", self.time.strftime("%H%M%S.%f") if self.time != None else None),
            convert_to_json("00080050", self.accession_number),
            convert_to_json("00080061", self.modalities),
