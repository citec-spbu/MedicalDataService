...
from app.edit.schemas import EditPatientData, EditDicomFileData, PatientInfo, DicomFileInfo
from app.edit.dao import EditDAO

router = APIRouter(prefix="/edit", tags=["Edit"])

def check_admin_role(user_data):
    """Проверка, что пользователь имеет роль администратора."""
    if user_data.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403,
            detail="Only administrators can edit or view this data"
        )

@router.get("/patient/{patient_id}", response_model=PatientInfo)
async def get_patient_info(patient_id: int, user_data = Depends(get_current_user_from_access)):
    """Получение информации о пациенте для редактирования."""
    check_admin_role(user_data)
    ...
    return PatientInfo(id=patient.id, name=patient.name, birth_date=patient.birth_date)

@router.put("/patient")
async def edit_patient(edit_data: EditPatientData, user_data = Depends(get_current_user_from_access)):
...
async def get_dicom_file_info(file_id: int, user_data = Depends(get_current_user_from_access)):
    ...
    return DicomFileInfo(
        ...
        upload_time=dicom_file.upload_time if dicom_file.upload_time else None,
        uploader_id=dicom_file.uploader_id
    )

@router.put("/dicom-file")
async def edit_dicom_file(edit_data: EditDicomFileData, user_data = Depends(get_current_user_from_access)):
